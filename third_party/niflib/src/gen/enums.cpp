...
ostream & operator<<( ostream & out, DataStreamAccess const & val ) {
	...
	for (int i = CPU_READ; i <= CPU_WRITE_STATIC_INITITIALIZED; i <<= 1 ) {
		switch ( ival & i ) {
			case CPU_READ:
				out << "CPU Read";
			break;
			
			case CPU_WRITE_STATIC:
				out << "CPU Write Static";
			break;

			case CPU_WRITE_MUTABLE:
				out << "CPU Write Mutable";
			break;

			case CPU_WRITE_VOLATILE:
				out << "CPU Write Volatile";
			break;
			...
			case CPU_WRITE_STATIC_INITITIALIZED:
				out << "CPU Write Static Inititialized";
			break;

			default: continue; // return out << "Invalid Value! - " << (unsigned int)(val);
		}
		ival &= ~i;
