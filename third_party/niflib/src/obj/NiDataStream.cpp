...
NiDataStream::NiDataStream() : usage((DataStreamUsage)-1), access((DataStreamAccess)-1), numBytes((unsigned int)0), cloningBehavior((CloningBehavior)CLONING_SHARE), numRegions((unsigned int)0), numComponents((unsigned int)0), streamable(1) {
...
void NiDataStream::Read( istream& in, list<unsigned int> & link_stack, const NifInfo & info ) {
	...
	// usage and access should be retrieve in the block type name "RTTI".
	if (usage < 0) {
