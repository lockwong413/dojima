...
vector<NiObjectRef> ReadNifList( istream & in, list<NiObjectRef> & missing_link_stack, NifInfo * info ) {
	...
	*info = header.Read( in );
	...
	cout << endl << "Reading Objects:" << std::endl;
#endif
	...
	while (true) {
		...
		// NOTE
		// NiDataStream can have "RTTI" info if finished by "\x01"
		// cerr << i << " :  " << objectType << endl;
